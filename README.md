...
- **非常に低負荷な処理** - ブラウザ上でのストリーミング処理により、長時間動画も効率的に処理しメモリ負担がほとんどないのが特徴です。
...
- **Chrome**: `NicoCommentDL-Chrome-バージョン名.zip` を解凍してから「パッケージ化されていない拡張機能を読み込む」で解凍したファイルを読み込むとインストールできます。
